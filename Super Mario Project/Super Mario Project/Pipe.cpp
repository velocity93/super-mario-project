	...
	void Pipe::update(RenderWindow& app)
    {
		if(_monster != nullptr)
		{
			if(_monsterExitDuration.GetElapsedTime() >= MONSTER_EXIT_TIME)
			{
				...
				{
				case TO_TOP:
                    monsterOccurrence = new MonsterOccurrence(_monster->getMonsterOccurrences().front()->getTexture()->getName(), Vector2f(_position.x * BLOCK_WIDTH + BLOCK_WIDTH / 2, _position.y * BLOCK_WIDTH));
				...
				monsterOccurrence->setState(MonsterOccurrence::M_GET_OUT_FROM_PIPE);
